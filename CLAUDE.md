...
- **Push-to-talk → Batch.** On release, the **post-gate `MediaRecorder` webm** uploads to **ElevenLabs Scribe v2 batch** (`handleTranscribeBatch`) and the accurate transcription lands on the clipboard. In batch the **noise gate is load-bearing**: the post-gate recording *is* what gets transcribed. Single-speaker medical config (`temperature=0`, `language_code=en`). **Diarization keep-primary-speaker is ON by default** (per-device `#diarize` toggle): when on, the Worker sends `diarize=true` (word-granular, no forced `num_speakers=1`) and the client (`keepPrimarySpeaker`) drops any non-dominant speaker — the lever against bystander voices the mic catches, since noise suppression / iOS Voice Isolation strip *noise* but another person's speech survives them. Removals are surfaced as a **no-beep status note** ("Filtered out N words from other speakers"); the filter can only ever *remove* a second voice, never empty a clean note (it no-ops on a single speaker or missing `words[]`). When off, the Worker reverts to `diarize=false`/`num_speakers=1`.
...
- **Boot restore**: `restoreLatestFromHistory()` puts the newest history entry into the box so the note stays visible across reloads; click-to-append (or append mode) can then extend it.
...
- Proxies to **ElevenLabs** `https://api.elevenlabs.io/v1/speech-to-text` batch `scribe_v2` with `xi-api-key` / `env.ELEVENLABS_API_KEY`. Single-speaker medical config (`temperature=0`, `language_code=en`). **`diarize` is now client-driven** (default on): `diarize=true` ⇒ word-granular, no forced `num_speakers`, and the client keeps the primary speaker; `diarize=false` ⇒ the old `num_speakers=1`/`diarize=false`. Multipart fields: `api_key` or `passphrase` (shared mode injects the master key server-side after the constant-time `safeEqual`), `file` (webm/ogg from the `MediaRecorder`), `file_format=other`, `timestamps_granularity`, `no_verbatim`, `tag_audio_events`, `diarize`, `keyterms_json` (≤ 1000 terms, < 50 chars). Size gates 1 KB–25 MB; failures return a string `error` that surfaces in the loud failure status.
...
# 30  diarization keep-primary-speaker: toggle defaults on + sends diarize=true,
#     a two-speaker words[] keeps only the dominant voice (bystander dropped) with
...
- **Phone link in batch:** the phone has no live text, so the desktop's `remoteCommitted`/`phone_session_end` grace-fallback never fires — the desktop relies entirely on the authoritative `phone_delivery` POST. That delivery is acked + replayed + deduped, and a phone that dies *before* delivering is now covered by the **phone-side delivery queue** (persisted + retried on heal/boot — see the Phone link section). The remaining audio-side gap is the in-flight *dictation* (audio not yet uploaded) when a phone dies mid-recording — the IndexedDB dictation journal (roadmap) is that answer; the queue covers the post-transcription delivery leg.
