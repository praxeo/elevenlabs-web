...
- **Realtime** — Mistral Voxtral realtime STT over WebSocket (`voxtral-mini-transcribe-realtime`); live text is the deliverable. (Was ElevenLabs Scribe v2 Realtime — swapped out. The client still speaks the ElevenLabs frame vocabulary; the Worker translates to/from Voxtral's protocol.)
...
- **Hybrid** — realtime text is *feedback* (Mistral Voxtral); the same audio is re-transcribed through batch Scribe v2 (**ElevenLabs**) and *that* lands on the clipboard.
...
- **WebSocket upgrade** → `handleTranscribeRealtime`: proxies to **Mistral** `wss api.mistral.ai/v1/audio/transcriptions/realtime` with `Authorization: Bearer`. The realtime path uses the **Mistral** key — BYO via the `api_key` query param, or shared-mode injects `env.MISTRAL_API_KEY` after the passphrase check. Session config is sent as a `session.update` frame once the socket opens: `model: voxtral-mini-transcribe-realtime`, `audio_format:{encoding:"pcm_s16le",sample_rate:16000}`, and `target_streaming_delay_ms` (Voxtral's only live tuning knob — latency vs accuracy — driven by the Advanced **Voxtral Realtime Filters** `streamingDelay` slider, additive `_v9` field, passed as the `target_streaming_delay_ms` query param). The remaining ElevenLabs-era query params the client still sends (`no_verbatim`, `timestamps`, `keyterms_json`) are **inert/not forwarded** (Voxtral realtime has no keyterm/no-verbatim equivalent). The Worker bridges the two protocols (see the translation note below).
- **POST** (multipart form) → `handleTranscribeBatch`: proxies to **ElevenLabs** `https://api.elevenlabs.io/v1/speech-to-text` batch `scribe_v2` with `xi-api-key`/`env.ELEVENLABS_API_KEY`. **Unchanged by the Mistral swap** — batch and the hybrid refine still hit ElevenLabs. Fields: `api_key` or `passphrase`, `file` (webm/ogg from batch mode, wav from the hybrid refine), `file_format=other`, `timestamps_granularity` (none/word/character), `no_verbatim`, `tag_audio_events`, `keyterms_json` (≤ 1000 terms, < 50 chars). Size gates 1 KB–25 MB.
- **Realtime frame translation** (`voxtralClientToBackend` + `makeVoxtralToClient`, exported for tests, top of `worker.js` above `handleTranscribeRealtime`): the client speaks the ElevenLabs frame vocabulary; the Worker maps it to Voxtral's wire protocol (verified against the `mistralai` SDK). Connection: model rides the **URL** (`?model=…`), auth is `Authorization: Bearer`; the Worker then sends a `session.update` whose config nests under `session` (`{audio_format, target_streaming_delay_ms}` — **no model**, sent before any audio). Client `input_audio_chunk` → `input_audio.append` (base64 in `audio`); the final flush (commit) → `input_audio.flush` then `input_audio.end`. Server → client: `session.created` → `session_started`; `transcription.text.delta` (`text`, incremental) accumulates into a running `partial_transcript`; `transcription.done` (`text`, full) emits **only the not-yet-committed portion** as a `committed_transcript` (handles per-segment *or* cumulative done text without double-counting). The `error` event (`error.message`) — and any other error-bearing event → a `{message_type:"error"}` frame so the loud path fires. The same normalized frames also feed the phone-link room (the desktop listener parses the ElevenLabs vocabulary too). Credentials: realtime needs the Mistral key, batch the ElevenLabs key; hybrid needs both (shared-mode passphrase covers both). BYO key fields: `mistral_api_key_browser_v9` (new, additive) and `elevenlabs_api_key_browser_v9`.
...
- Client → server WS frames: every chunk goes through the `sendAudioChunk` chokepoint: `{"message_type":"input_audio_chunk","audio_base_64":"…","commit":false,"sample_rate":16000}`; the final flush sets `commit:true`. `previous_text` (append-continuation context) may ride **only the first** chunk — but **Voxtral realtime has no `previous_text` equivalent, so the Worker drops it** (cross-press continuity drift, like the batch refine already has). This is the *client* contract; the Worker translates it to Voxtral's `input_audio_buffer.*` events.
- Server → client frames (the vocabulary the **client and phone listener** consume; the Worker emits these by translating Voxtral events): `session_started` (config now empty — Voxtral doesn't echo keyterms, so the "(N keyterms active)" status enhancement just doesn't appear), `partial_transcript`, `committed_transcript`. Client rule: **any frame carrying a string `error` takes the loud error path** — never match error types by name only. The Worker synthesizes an `error` frame then closes `1008` on handshake failures, and maps any Voxtral error event to one too.
...
# strip, per-device override, joined local-copy denial deferring to the
# relay ack, Mistral Voxtral realtime frame translation: client chunk→
# append/commit, delta accumulation→running partial, done→committed
# (per-segment + cumulative dedupe), error event→loud error frame):
