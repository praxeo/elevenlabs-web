...
- **WebSocket upgrade** → `handleTranscribeRealtime`: proxies to **Mistral** `wss api.mistral.ai/v1/audio/transcriptions/realtime` with `Authorization: Bearer`. The realtime path uses the **Mistral** key — BYO via the `api_key` query param, or shared-mode injects `env.MISTRAL_API_KEY` after the passphrase check. There are **no backend query params**: session config (`model: voxtral-mini-transcribe-realtime`, `audio_format:{encoding:"pcm_s16le",sample_rate:16000}`) is sent as a `session.update` frame once the socket opens. The client still sends ElevenLabs-era query params (`no_verbatim`, `timestamps`, `vad_*`, `keyterms_json`) — they are **inert/not forwarded** (Voxtral realtime has no keyterm/VAD-tuning equivalent). The Worker bridges the two protocols (see the translation note below).
