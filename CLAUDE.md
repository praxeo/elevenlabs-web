...
- **Realtime frame translation** (`voxtralClientToBackend` + `makeVoxtralToClient`, exported for tests, top of `worker.js` above `handleTranscribeRealtime`): the client speaks the ElevenLabs frame vocabulary; the Worker maps it. Client `input_audio_chunk` → `input_audio_buffer.append` (base64 in `audio`) + `input_audio_buffer.commit{final:true}` on the final flush. Voxtral `transcription.delta` (`delta`) accumulates into a running `partial_transcript`; `transcription.done` (`text`) emits **only the not-yet-committed portion** as a `committed_transcript` (handles per-segment *or* cumulative done text without double-counting). **Any** error-bearing backend event → a `{message_type:"error"}` frame so the loud path fires. The same normalized frames also feed the phone-link room (the desktop listener parses the ElevenLabs vocabulary too). Credentials: realtime needs the Mistral key, batch the ElevenLabs key; hybrid needs both (shared-mode passphrase covers both). BYO key fields: `mistral_api_key_browser_v9` (new, additive) and `elevenlabs_api_key_browser_v9`.
