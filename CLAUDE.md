...
- **Realtime** — **Deepgram Nova-3 on Cloudflare Workers AI** (`@cf/deepgram/nova-3` via the `env.AI` binding, `mode:"medical"`); live text is the deliverable. Runs ON Cloudflare's edge — no external hop. (Engine history: ElevenLabs Scribe v2 Realtime → Mistral Voxtral → Soniox `stt-rt-v5` → Nova-3, each swapped out. The client still speaks the same frame vocabulary; the Worker translates to/from Deepgram's protocol via `novaClientToBackend` / `makeNova3ToClient`.) **⚠ VERIFY-ON-LIVE:** Cloudflare does not publish nova-3's streaming frame shape or `sample_rate` support — the translator is defensive across 3 shapes and surfaces an unrecognized frame loudly; confirm the real shape + that `mode:medical`/`sample_rate` apply against a live `env.AI` binding before trusting it for charts.
...
- **Hybrid** — realtime text is *feedback* (Nova-3); the same audio is re-transcribed through batch Scribe v2 (**ElevenLabs**) and *that* lands on the clipboard.
...
- **WebSocket upgrade** → `handleTranscribeRealtime`: streams to **Deepgram Nova-3 on Workers AI** via `env.AI.run("@cf/deepgram/nova-3", cfg, {websocket:true})` — the returned `aiResp.webSocket` is the backend socket (no outbound `fetch`, no external hop). **No STT key**: access is gated only by the shared-mode passphrase (so the billed AI binding can't be driven anonymously); single-tenant deploys need no realtime credential. The `cfg` (second arg, NOT a query string): `{encoding:"linear16", sample_rate:"16000", channels:1, language:"en-US", mode:"medical", interim_results:true, smart_format:true, punctuate:true, numerals:true, endpointing:"false", keyterm:<space-joined terms>}`. Note `mode:"medical"` (NOT a `model` field — that doesn't exist in the schema), `endpointing:"false"` (don't split continuous PTT), and `sample_rate`/streaming-frame-shape are **verify-on-live** (absent from nova-3's published schema). `no_verbatim`/`timestamps` query params remain **inert/not forwarded**.
...
- **Realtime frame translation** (`novaClientToBackend` + `makeNova3ToClient`, exported for tests, top of `worker.js` above `handleTranscribeRealtime`): the client speaks the same frame vocabulary; the Worker maps it to Deepgram's wire protocol. Client `input_audio_chunk` → **raw PCM bytes** (base64-decoded `Uint8Array`, binary WS frame); the final flush (commit) → `{"type":"Finalize"}` + `{"type":"CloseStream"}` (Deepgram end-of-stream). An `inputEnded` latch drops stray post-commit frames; a `{"type":"KeepAlive"}` every 5 s avoids the ~10 s idle close. Deepgram → client: `makeNova3ToClient` is **defensive across 3 unverified shapes** — (A) native `{type:"Results", is_final, speech_final, from_finalize, channel.alternatives[0].transcript}`, (B) batch `{results.channels[0].alternatives[0].transcript}`, (C) Flux `{event, transcript}` — accumulating locked text into a running `partial_transcript`, emitting `committed_transcript` on `from_finalize`/`speech_final`/`Metadata`/`EndOfTurn`. **ANY string error → loud `{message_type:"error"}`; an UNRECOGNIZED first frame is surfaced loudly** (so the real nova-3 streaming shape is observed on the first live test rather than silently producing a blank/wrong chart). If Workers AI doesn't honor the control frames, the client's `FINAL_WAIT_MS`/trailing-partial path still delivers — a missed finalize never loses text. Credentials: realtime needs none (Nova-3 = `env.AI`); batch/hybrid-refine need the ElevenLabs key (or shared passphrase). The `soniox_api_key_browser_v9` field is now **vestigial/hidden**.
...
- Server → client frames (the vocabulary the **client and phone listener** consume; the Worker emits these by translating Deepgram responses): `session_started` (config empty), `partial_transcript`, `committed_transcript`. Client rule: **any frame carrying a string `error` takes the loud error path** — never match error types by name only. The Worker synthesizes an `error` frame then closes `1008` on handshake failures, and maps any Deepgram error frame (or abnormal close) to one too.
...
`npx wrangler deploy` (worker name `eleven` — the pre-merge batch app's URL, so its users' localStorage survives; secrets persist across deploys). Realtime (Nova-3) needs the **`[ai]` binding** in `wrangler.toml` (`binding = "AI"`) and a Workers-AI-enabled account — **no STT secret**. Shared mode needs: `npx wrangler secret put ELEVENLABS_API_KEY` (batch/refine) and `…put APP_PASSPHRASE` (gates realtime + batch). HTML is served `cache-control: no-store`; manifest/icons are cacheable. `wrangler dev` works for UI checks; **the realtime path hits the real billed `env.AI` binding even in `wrangler dev`** — there's no local STT stub.
