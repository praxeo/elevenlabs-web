...
- **WebSocket upgrade** → `handleTranscribeRealtime`: proxies to **Soniox** `wss stt-rt.soniox.com/transcribe-websocket`. Auth is **not** a header — the Soniox key rides the first (config) frame, so the upgrade carries no credential. The realtime path uses the **Soniox** key — BYO via the `api_key` query param, or shared-mode injects `env.SONIOX_API_KEY` after the passphrase check. The Worker sends the JSON config as the first frame: `{api_key, model:"stt-rt-v5", audio_format:"pcm_s16le", sample_rate:16000, num_channels:1, language_hints:["en"], enable_endpoint_detection:true, endpoint_sensitivity:-0.5, context:{general:[{key:"domain",value:"Healthcare"}], terms:[…keyterms…]}}`. `endpoint_sensitivity:-0.5` is Soniox's documented dictation tuning (rare endpoints, so the model keeps revising through clause-level pauses); the **always-on** `context.general` Healthcare domain hint biases every dictation toward medical vocabulary even when the user has no custom keyterms. **Realtime keyterms are back** (Voxtral lacked them) — sent as `context.terms` (sanitized: ≤ 100 terms). The Worker also sends `{type:"keepalive"}` every 10 s while audio is not flowing (gated on `!inputEnded`, cleared on close) so Soniox's ~20 s idle close can't drop a paused/connecting socket. The other ElevenLabs-era query params the client still sends (`no_verbatim`, `timestamps`) are **inert/not forwarded**. The Worker bridges the two protocols (see the translation note below).
...
- **Realtime frame translation** (`sonioxClientToBackend` + `makeSonioxToClient`, exported for tests, top of `worker.js` above `handleTranscribeRealtime`): the client speaks the ElevenLabs frame vocabulary; the Worker maps it to Soniox's wire protocol. Client `input_audio_chunk` → the **raw PCM bytes** (base64-decoded into a `Uint8Array`, sent as a binary WS frame — Soniox takes binary, not base64); the final flush (commit) → an **empty string** (`""` = end-of-audio). An `inputEnded` latch drops any stray post-commit frame. Soniox → client: each response carries `tokens[]` with an `is_final` flag — **non-final** tokens are provisional (resent each response), **final** tokens are confirmed (sent once). The Worker accumulates the confirmed text and emits, every response, a `partial_transcript` holding `confirmed + provisional-tail` (the complete current view); on `finished` it emits a `committed_transcript` with the confirmed text (locks in + triggers the client's prompt close). Control tokens `<end>` (endpoint) and `<fin>` (manual finalize) are dropped by **exact match** — never a regex, which could eat real angle-bracket text. An `error_code` **or** `error_type` response → a `{message_type:"error"}` frame keyed off the stable `error_type` slug with an actionable plain-English lead (auth / quota / rate-limit) and the raw `error_message` appended, so the loud path fires; an abnormal backend close surfaces the transport close code as an error frame too (documented Soniox errors arrive as a JSON error frame then a 1000 close — the close-code path is the 1006-style transport-drop backstop). The same normalized frames also feed the phone-link room (the desktop listener parses the ElevenLabs vocabulary too). Credentials: realtime needs the Soniox key, batch the ElevenLabs key; hybrid needs both (shared-mode passphrase covers both). BYO key fields: `soniox_api_key_browser_v9` and `elevenlabs_api_key_browser_v9`.
...
- Server → client frames (the vocabulary the **client and phone listener** consume; the Worker emits these by translating Soniox responses): `session_started` (**Worker-synthesized** — Soniox sends no config echo — carrying `{model, keyterms}` so the status line shows *(N keyterms **sent**)*: client-side intent, **not** a server ack, since Soniox never confirms applied keyterms), `partial_transcript`, `committed_transcript`. Client rule: **any frame carrying a string `error` takes the loud error path** — never match error types by name only. The Worker synthesizes an `error` frame then closes `1008` on handshake failures, and maps any Soniox `error_code` (or abnormal close) to one too.
