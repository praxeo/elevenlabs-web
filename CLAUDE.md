...
- **Mic-tips onboarding (keep other voices out)**: bystander speech in a note is the "silent wrong text" failure, and **iOS Voice Isolation** (Control Center → Mic Mode → Voice Isolation — filters other voices at the OS level, before audio reaches the page) is the single most effective lever — but it's manual and undetectable from the web. So `#micTips` is a one-time nudge (`showMicTips`/`closeMicTips`/`maybeAutoShowMicTips`): the iOS Voice-Isolation steps (shown when `isLikelyIOS()`, an Android note otherwise) plus the universal close-mic + push-to-talk-discipline habits. **Auto-shows once on the big-button surface** (in `applyBigButtonUI`), dismissal persists the additive per-device `micTipsSeen` field (no re-nag), and it's reopenable from the big-button top row and Options. This is the shipped first step of the other-voice-rejection work; the heavier lever (Scribe diarize + keep-primary-speaker, *loud about what it removed*) is still open.
