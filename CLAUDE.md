...
- **Hybrid** — **THE REAL PRODUCTION DELIVERABLE.** Soniox live text is *feedback for the clinician's eyes*; the same captured PCM is re-transcribed through **ElevenLabs Scribe v2 batch** and *that* lands on the clipboard (batch-tier accuracy, ~a tier above any streaming engine). The live-vs-batch accuracy gap is **inherent to streaming** (no right-context) — do not expect any realtime engine's live text to reach batch accuracy; that is exactly why hybrid exists. (`DEFAULT_ENGINE` is still `"batch"`; flipping it to `"hybrid"` only affects fresh profiles.)

Users are clinicians doing push-to-talk dictation into Cerner/Citrix via AutoHotkey, with clipboard handoff. See `README.md` for product behavior, `REALTIME_HANDOFF.md` for the realtime engine investigation log, and `LATENCY_PLAN.md` for the open finalize-latency work.
...
- `worker.js` — almost everything: Worker fetch handler (dual-protocol `/api/transcribe` with the realtime transport selector, the per-engine translators, the temp `/api/nova-probe`, the `/pcm-pump.js` worklet route), batch proxy, WebSocket proxy, PWA manifest/icons, and the entire client app embedded in the `INDEX_HTML` template literal.
...
- `REALTIME_HANDOFF.md` — the realtime-engine investigation log (root causes, what was ruled out, the engine evolution). `LATENCY_PLAN.md` — the open plan to cut post-speech finalize latency.
             ...
             └─ open: flush buffer, stream live
                 └─ stopRecording() [PTT release]: stopPhase="tail" — audio KEEPS
                    streaming for TAIL_MS (anti-clipping)
                     ├─ ENGINE realtime: beginCommitPhase(): send
                     │  {audio_base_64:"", commit:true}, stopPhase="awaitFinal";
                     │  close COMMIT_QUIET_MS after the last committed transcript,
                     │  or at FINAL_WAIT_MS deadline → onclose →
                     │  finalizeSession → deliverFinalText(live text)
                     └─ ENGINE hybrid (FAST FINALIZE): after TAIL_MS, do NOT wait
                        for the realtime engine's finalize — sessionPcm is already
                        complete. Commit + close the live socket in the background
                        and go straight to finalizeSession → refineAndDeliverHybrid():
                        buildWavBlob(sessionPcm) → batchTranscribe (REFINE_TIMEOUT_MS)
                        → refined text is the clipboard → deliverFinalText;
                        refine-fail → live text + warnBeep; both-fail → sentinel
```

- **Hybrid is the production deliverable.** The realtime (Soniox) text is on-screen feedback only; the ElevenLabs batch re-transcription of the captured PCM is what lands on the clipboard. The hybrid path was changed (2026-06) so the batch refine starts the instant the tail ends — it no longer blocks on the realtime engine's finalize (that was dead time). The remaining post-speech latency is the batch refine itself (~2–4 s); see `LATENCY_PLAN.md` for the open work.
...
- **`POST /api/soniox-token`** → `handleSonioxToken`: passphrase-gated; mints a Soniox **temporary API key** (`usage_type:"transcribe_websocket"`, `single_use`, `expires_in_seconds:120`, `max_session_duration_seconds:600`) so the client can open the realtime WS **directly** to Soniox (`?rt=direct`) with no Worker proxy hop. The long-lived `SONIOX_API_KEY` never leaves the Worker. In direct mode the client builds the Soniox config + translates tokens itself (`buildSonioxDirectConfig` / `makeSonioxClientTranslator`, mirroring the Worker's versions); audio is **binary PCM** straight to Soniox; a mint failure **falls back to the proxy transport** (never a dead mic). Joined phones add a `SessionRoom` `?role=pub` publisher socket to mirror frames to the desktop (the room emits `phone_session_end` on publisher disconnect). See `REALTIME_HANDOFF.md` Session 4.
- **WebSocket upgrade** → `handleTranscribeRealtime`: a **transport selector** keyed off `?rt=` chooses the realtime backend (the client forwards `?rt=` from the page URL; default/absent = Soniox proxy; `?rt=direct` bypasses this handler entirely — see the token endpoint above). Shared mode gates all of it on the passphrase. All proxy transports emit the SAME client frame vocabulary:
  - **`soniox` / `auto` (DEFAULT)** — `wss://stt-rt.soniox.com/transcribe-websocket`, opened from the Worker via `fetch(url,{headers:{Upgrade:"websocket"}}).webSocket`. Auth + config ride the **first JSON frame** (the Worker sends it, not the client): `{api_key:env.SONIOX_API_KEY, model:"stt-rt-v5", audio_format:"pcm_s16le", sample_rate:16000, num_channels:1, language_hints:["en"], language_hints_strict:true, enable_endpoint_detection:false, context:{general:[…], terms:[…]}}`. **`enable_endpoint_detection:false`** (PTT supplies the explicit end-of-audio; endpointing would finalize mid-sentence and lose right-context). Audio = **raw binary PCM frames**; commit = an **empty-string** frame (end-of-audio). Translators: `sonioxClientToBackend` (base64→binary, commit→`""`) + `makeSonioxToClient` (token `is_final`→accumulate, running `partial_transcript`, `committed_transcript` on `finished`, drops `<end>`/`<fin>` markers, `error_code`→loud error). Soniox finalizes in **~75 ms** after commit — the engine is NOT the post-speech latency; the client constants are (see `LATENCY_PLAN.md`).
  - **`el` / `scribe`** — ElevenLabs Scribe v2 Realtime: `wss://api.elevenlabs.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&audio_format=pcm_16000&language_code=en&commit_strategy=manual&keyterms=…`, `xi-api-key` handshake header. **Near-identity passthrough** — the client vocabulary was modeled on EL's native realtime protocol, so the Worker forwards JSON both ways (dropping `previous_text` past the first chunk); the client's empty-audio `{commit:true}` IS EL's commit. Batch-quality accuracy but ~2 s first-word / ~1/sec interims.
  - **`binding` / `nova`** — Deepgram Nova-3 on Workers AI via `env.AI.run("@cf/deepgram/nova-3", cfg, {websocket:true})` (all cfg values STRINGS; the binding 500s on booleans). Translators `novaClientToBackend` / `makeNova3ToClient`. RELEGATED: the binding floored interim cadence at ~1/sec and `sample_rate` is absent from its schema. **`flux`** = `@cf/deepgram/flux`; **`gw`/`dgw`** = AI Gateway `workers-ai`/`deepgram` passthrough (need `CF_ACCOUNT_ID`+`CF_AIG_GATEWAY`+`CF_AIG_TOKEN`, and `DEEPGRAM_API_KEY` for `dgw`→`nova-3-medical`); inert until those secrets exist.
- **POST** (multipart form) → `handleTranscribeBatch`: proxies to **ElevenLabs** `https://api.elevenlabs.io/v1/speech-to-text` batch `scribe_v2` with `xi-api-key`/`env.ELEVENLABS_API_KEY`. **This is the hybrid clipboard deliverable AND pure-batch mode** — top accuracy, already locked to the best single-speaker medical config (`temperature=0`, `language_code=en`, `num_speakers=1`, `diarize=false`). Fields: `api_key` or `passphrase`, `file` (webm/ogg from batch mode, wav from the hybrid refine), `file_format=other`, `timestamps_granularity`, `no_verbatim`, `tag_audio_events`, `keyterms_json` (≤ 1000 terms, < 50 chars). Size gates 1 KB–25 MB.
- **Translators are exported for tests** (top of `worker.js` above `handleTranscribeRealtime`): `sonioxClientToBackend`, `makeSonioxToClient`, `novaClientToBackend`, `makeNova3ToClient` (EL needs none — passthrough). All map the client vocabulary ↔ the engine wire protocol.
...
# fallback path via the same handleAudioFrame; DIRECT stream (?rt=direct):
# temp-key mint, WS straight to Soniox, config-first, binary PCM frames,
# on-device token translation + marker drop, empty-string commit,
# committed→clipboard, mint-failure→proxy fallback, and a joined publisher
# mirror (SessionRoom ?role=pub broadcast + phone_session_end on disconnect)):
...
**Deploy = push to `main`** — Cloudflare **Workers Builds auto-deploys** on push (worker name `eleven` — the pre-merge batch app's URL, so users' localStorage survives; secrets persist across deploys). Confirm via `gh api repos/praxeo/elevenlabs-web-/commits/<sha>/check-runs` (the `Workers Builds: eleven` check). `npx wrangler deploy` also works but the local OAuth token is **scope-limited**: it CANNOT write secrets (`auth error 10000`) or run `wrangler dev`'s remote AI proxy — **set secrets via the Cloudflare dashboard** (Workers & Pages → eleven → Settings). Secrets in use: `SONIOX_API_KEY` (realtime default), `ELEVENLABS_API_KEY` (batch/hybrid + `?rt=el`), `APP_PASSPHRASE` (gates everything in shared mode); optional `CF_ACCOUNT_ID`/`CF_AIG_GATEWAY`/`CF_AIG_TOKEN`/`DEEPGRAM_API_KEY` (the `?rt=gw|dgw` paths), `PROBE_KEY` (the temp diagnostic). The Deepgram `?rt=binding` path needs the **`[ai]` binding** (`binding = "AI"` in `wrangler.toml`). HTML + `/pcm-pump.js` served `no-store`/cacheable respectively; **the realtime path hits the real billed engine even in `wrangler dev`** — no local STT stub.

## Known sharp edges

- The frame pump AudioWorklet is served from a **real route `/pcm-pump.js`**, NOT a Blob URL — the Blob form silently failed to register on real browsers (`addModule` resolved but `new AudioWorkletNode` threw "not defined"), forcing the starving ScriptProcessor and garbling realtime for the whole Nova-3 era. `buildPumpNode` adds the module **once per AudioContext** + **retries node construction** across the registration race (Blob fallback, then ScriptProcessor). Keep all per-frame logic in `handleAudioFrame` (shared by all paths). **Mock-only tests can't catch a real-browser worklet failure** — the s27 scenario mocks `addModule`/`AudioWorkletNode`, which is exactly why this shipped broken; verify the worklet on a real device (`?debug=1` logs `[audio] AudioWorklet pump active`).
