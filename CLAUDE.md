...
- **WebSocket upgrade** → `handleTranscribeRealtime`: a **transport selector** keyed off `?rt=` chooses the realtime backend (the client forwards `?rt=` from the page URL; default/absent = Soniox). Shared mode gates all of it on the passphrase. All transports proxy through the Worker and emit the SAME client frame vocabulary:
