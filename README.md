...
Hold a key, speak, release. The audio uploads to ElevenLabs Scribe v2 Medical (batch) and the transcript lands on the clipboard. Batch-only, single speaker, medical config. On Chrome/Edge the upload streams while you are still talking, so the release only waits for the transcript (Advanced → "Upload while you dictate").
...
For setups where the desktop can't get a good mic. Pairing is named from both ends, because the same page runs on both devices: on the desktop, **📱 Pair a phone** shows a QR + 6-char code; on the phone, **🖥 Pair to a desktop** (on the main card, and in the top row of the big-button screen) is where you type that code, or you just scan the QR. (The QR is generated on-page, never by an external service — the code routes to the room, and in shared mode the **access code guards it**: every session route requires the same access code the app uses, so a guessed 6-char code alone can neither read a note nor plant one on the clipboard). The phone joins, switches to a **big-button layout** (one push-to-talk button, whole-screen status, haptics mirroring the beeps), and dictates in batch; the final text POSTs to the desktop, which writes the clipboard. While the phone is dictating, the desktop shows a live **"📱 Phone is recording…"** indicator (a pulsing dot that flips to "transcribing…" on release and clears when the text lands) so I know audio is flowing before the note arrives — it's a relayed, fire-and-forget cue, never load-bearing. Pairing survives reloads and app restarts on both sides until End session (desktop) / Leave (phone). The resilience machinery (heartbeat, reconnect, delivery queue, dedupe, focus-retry, and the AHK `/latest` poller for focus-free pasting) is detailed in `CLAUDE.md`. If you use the AHK poller against a shared-mode worker, set `PHONE_AUTH` (the access code) at the top of `hotkey.ahk` — the routes answer 401 without it.
...
1. **Durability — done, now including a failed upload.** **A timed-out or failed transcription upload no longer costs the dictation** (2026-09): the recording is kept instead of being discarded with the text, and the take is offered for **retry** — a banner on the desktop card, a tappable chip on the phone's big-button screen — which survives a relaunch, so a clinician never redictates from memory because the network hiccuped. The upload **deadline is now size-aware as well as duration-aware** (the phone-link timeouts were the upload leg: the ElevenLabs request log showed zero failures and a 0.58 s median, i.e. the service never saw the aborted takes), and a failed take's timing row now records the **stage reached, error class and the deadline it blew** instead of a row of blanks. The dictation journal landed: each recorded chunk is mirrored to IndexedDB during capture (~30 min cap, and a capped recovery says so), and a take that the app died on (crash / kill / OOM before upload) is recovered on the next boot — a loud banner re-transcribes the saved audio and copies it. The **transcript-coverage guard** closes the last silent gap: a result that stops well short of the observed speech (service truncation or a gate-silenced tail) is a loud "may be INCOMPLETE" warn — never a clean Done! — and logs a diagnosis to the failure ring (`scribe_v2_micfail_v9`). Long takes get a duration-scaled transcription deadline instead of the flat 15 s. *Next:* a UI to browse/replay the failure log and recovered audio.
2. **Phone-link delivery durability — done end-to-end, and the phone stands alone.** Dictating on the phone never needed the desktop, but it used to be *reported* as a failure whenever no desktop was listening: a red screen and a warn beep on every take, even though the note was transcribed, saved and queued. As of 2026-09 that is a **success** — a done beep and a status that says plainly it is queued and has not reached the desktop yet — while a desktop that *was* listening and then vanished still raises a single alarm. So you can dictate a whole shift on the phone with the desktop shut and push the notes over later. A note dictated while the desktop link was down can no longer be stranded: queued deliveries are code-bound and **survive Leave/rejoin** (re-linking with the same code delivers them immediately), a **"⏳ N waiting" chip** on both layouts makes the queue visible (tap to retry), and **"📤 Send to desktop"** (on the box, history rows, and the phone's expanded transcript peek) re-sends any past note on demand. Server-side, the room now **persists a ring of recent deliveries for 30 minutes** (survives Durable Object eviction, replayed on reconnect), the desktop **polls `/latest` as a fallback** whenever its WebSocket is down (a dead socket delays a note by at most ~a minute instead of losing it), and the phone's big screen shows a **"🖥 Desktop ✓ / not listening" pill** *before* you dictate (a positive check also delivers queued notes instantly).
3. **Phone-link security — access-code gate landed.** The session routes (`/deliver`, `/latest`, `/status`, the listener WebSocket) now require the shared access code whenever the worker has one configured, with **two-layer per-IP rate limiting** (Cloudflare's native global Workers rate-limit binding over a per-isolate backstop — both bound guess-scanning and DO-cost abuse, and the binding fails open so a plan without it still deploys) and a delivery size cap; a missing/wrong code is a loud, named failure on both surfaces, and the refused note stays queued. *Next:* end-to-end encryption of deliveries (key in the QR fragment, ciphertext-only relay) so patient text never rests readable on the room.
4. **Settings portability.** Sync the portable settings across devices while keeping per-device tuning local — export/import or passphrase-keyed server-side profiles.
5. **Phone UI — mic reliability after a resume, fixed.** The "mic gets stuck rebuilding" failure is gone (2026-09): an iOS resume fires three lifecycle events at once and each used to start its own microphone re-acquisition, leaving extra live streams that iOS starves, so the surviving one delivered pure silence and the app rebuilt in a loop until it was relaunched. Rebuilds are now serialized and superseded ones clean up after themselves; the between-takes self-heal is bounded and tells you to press the button instead of grinding; a mic that wakes a moment after a rebuild now records instead of being refused; and a pre-capture failure reports the level the probe actually measured rather than the previous take's. The big-button layout is done; surfacing the capture waveform on the big screen is the open gap (the Mic ✓/⚠ health pill landed), then a larger live readout and swipe gestures. **Pairing and link-state UX pass (2026-09):** pairing is named from both ends ("📱 Pair a phone" beside "🖥 Pair to a desktop", the latter also in the big-button top row for a solo phone); a phone that leaves tells the desktop (its "Phone paired ✓" no longer outlives the phone); a dropped listener link that heals clears its red instead of leaving a stale FAILED; the phone is told in words when its desktop stops listening, with the next step; joining, leaving and a healed link never paint the screen DONE (DONE means a dictation reached the clipboard); the phone screen shows phone copy instead of the desktop's Ctrl+Space hint; Settings from the big screen lands on Options; a permission-denied mic names the fix; and the batch-era copy (API key "(batch)", "engine", the retired filter explainer) is gone.
6. **Batch latency, mic accuracy, and the minimized-window recording cue.** Phases 0 and 1 of **`PERF_PLAN.md`** have **landed**: every take now records a per-stage latency breakdown to a per-device ring (with a `Server-Timing` split separating our transport from ElevenLabs inference, readable and exportable from Advanced); the whole window colours itself red while recording — plus tab title, favicon and taskbar badge — so a PWA shrunk beside Cerner on a machine with no speakers is readable at a glance; `hotkey.ahk` hands focus back to the window you dictated from (ending the alt-tab that cost more than the entire transcription pipeline) and shows an always-on-top state bar that works even when the app window is fully covered; and a gate look-ahead plus release tail stop the gate clipping the first and last words. Transcription now runs on **Scribe v2 Medical** (`scribe_v2_medical`, 2026-09), ElevenLabs' clinical fine-tune of Scribe v2 (drug names, anatomy, pathology): same request, same price, and every other setting is unchanged. **The upload now happens while you dictate** (2026-09, Chrome/Edge, ported from WhisperInk): the request to ElevenLabs opens when you press and the audio streams up as you speak, so letting go waits only for the transcript. It is used only when the streamed audio matches the recording byte for byte; anything else falls back to the normal upload on its own, and the timing log names which path each take used. Verified in production on 2026-09-24: long dictations now land about 0.4 s after release however long they are (two 19-second takes: 391 and 422 ms), our own share of the wait is under 40 ms, and the rest is ElevenLabs' processing. Short takes are still being measured. *Next:* an A/B of short takes with streaming on vs off, then the mic levers (device picker, narrowband warning, gate-bypass A/B). PCM upload is deprioritized: it would put a new audio processor in the recording path to save part of ElevenLabs' decode time.
