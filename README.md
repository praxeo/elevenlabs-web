...
- **One note per dictation by default.** Hit **➕ Append next** to add the next dictation onto the current note (a one-shot), or turn on append mode to keep chaining every dictation onto the note until you turn it off or clear the box. Append is always explicit — there's no hidden timer that decides for you. The most recent transcript is restored on reload, and the clipboard always holds the current note.
