...
- [x] **Realtime accuracy & diagnosability tuning** (verified against Soniox's live docs): Soniox's own dictation tuning — `endpoint_sensitivity:-0.5` (rare endpoints, so the model keeps revising through clause-level pauses) and an **always-on `context.general` Healthcare domain hint** (medical-vocabulary bias on every dictation, even with no custom keyterms); a 10 s backend **keepalive** so a paused/connecting socket can't hit Soniox's ~20 s idle close; **actionable error messages** keyed off Soniox's stable `error_type` (auth / quota / rate-limit, not a bare `401`); a working **"(N keyterms sent)"** status (Soniox doesn't echo config, so the Worker synthesizes it from what it sent — intent, not a server ack); exact-match `<end>`/`<fin>` control-token filtering; and a live-display repaint dedupe so a long note doesn't re-render the whole growing string ~12×/s on a phone. **Caveat:** config bias is probabilistic — Soniox's raw medical-English word accuracy is a model property no setting guarantees, so **hybrid** (ElevenLabs batch as the clipboard/chart deliverable) stays the safety net for the live leg.
