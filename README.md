...
- **Re-transcribe (batch)** button: re-runs the **last dictation's audio** through the accurate batch `scribe_v2` model and replaces the note text with the result (recopied to the clipboard, relayed to the desktop if you're on a joined phone). It appears after any dictation that captured audio and is the answer to two situations: a **failure** (the upload/connection died — retry it), and a **realtime result too inaccurate to trust** (upgrade it to batch quality without re-dictating). The audio is whatever the dictation captured — the ungated WAV in realtime/hybrid, the recorded webm in batch — held until your next dictation. A failed re-transcribe keeps the existing text (it never overwrites a real transcript with the sentinel).
...
- [x] **Re-transcribe last dictation**: a button that re-runs the previous dictation's retained audio (ungated WAV in realtime/hybrid, recorded webm in batch) through the accurate batch model — a recovery path after a failed upload/connection and an accuracy upgrade when the live realtime text can't be trusted. Goes through the single delivery exit (one clipboard write, one beep, relayed if joined); a failed retry keeps the existing text. A first, in-memory step toward the durability journal's one-tap recovery below.
