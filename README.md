...
- **Turn on Voice Isolation.** While the mic is active, open Control Center → **Mic Mode** → **Voice Isolation**. iOS suppresses background voices at the OS level, before the audio ever reaches the page — by far the most effective fix for ambient speech ending up in notes, and it persists per app. The app now **walks you through this** with a one-time **Mic tips** card on the phone (reopen it any time from the big-button top row or **Options → Mic tips**); it also covers the close-mic and push-to-talk habits below.
...
| ~~Remove ellipses / Strip newlines / Trailing space / Remove filler words~~ | always on | all | These were toggles; they're now **permanently on** (the only sensible shape for pasting into Cerner) and the checkboxes were removed. Scribe's "…"/"..." pause markers are stripped, newlines collapse to spaces, a trailing space is appended, and `no_verbatim` (filler/false-start removal) always rides the batch call. |
...
- [x] **Phone-side delivery queue** — a phone whose `/deliver` POST fails or acks **zero listeners** (desktop down past the 2-minute server buffer) no longer relies on that buffer alone: the delivery is **persisted to `localStorage` and retried** on link heal (`online`/visibility/focus, a backoff timer) and **at boot** (a phone that died after transcribing but before delivering recovers on relaunch), until a desktop listener acks it. Each retry reuses the **same `delivery_id`**, and the desktop now dedupes against a **ring of recent ids** (not just the last one), so a retried or out-of-order re-POST can never re-copy stale text onto a chart. Capped (size + a 30-min staleness drop) so the queue can't grow unbounded or auto-paste an hours-old note. Per-device. (Scenario 20q.)
